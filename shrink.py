...
print "%s channels will be included in the new EGP" % len(excluded_ids)
#print excluded_ids
...
for p in programmes:
  if p.get('channel') not in excluded_ids:
    root.remove(p)

#Write the output file 
tree.write(small_epg_file, 'utf-8', True)   

#with gzip.open(small_epg_zip_file, 'wb') as f:
#  f.write(content)
with open(small_epg_file, 'rb') as f_in, gzip.open(small_epg_zip_file, 'wb') as f_out:
  shutil.copyfileobj(f_in, f_out)
    
#z = zipfile.ZipFile(small_epg_zip_file, 'w')
#z.write(small_epg_file)
#z.close()
